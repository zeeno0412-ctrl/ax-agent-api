...
const VERDICT_CONFIG: Record<string, { label: string; icon: string; title: string; desc: string; bg: string; border: string; text: string; badge: string }> = {
...
function parseNotionPage(page: Record<string, unknown>): Receipt {
  const p = (page.properties || {}) as Record<string, Record<string, unknown>>;
  const text = (prop: Record<string, unknown> | undefined): string => {
    ...
    return titleArr?.[0]?.plain_text || richArr?.[0]?.plain_text || "";
  };
  const sel = (prop: Record<string, unknown> | undefined): string => {
    const selectObj = prop?.select as { name: string } | undefined;
  ...
  return {
    ...
    reason:    "",
    firstMsg:  title,
    createdAt: ((p["접수일시"]?.date as { start?: string })?.start) || (page.created_time as string),
  };
}
...
async function evaluatePOVWithAI(answers: Answers) {
  const summaryLines = [
...
function ChatBubble({ from, children }: { from: string; children: ReactNode }) {
...
function POVCard({ answers }: { answers: Answers }) {
  return (
...
function DeepenModal({ answers, pov, receiptId, onClose }: { answers: Answers; pov: string; receiptId: string; onClose: () => void }) {
  ...
  function saveEdit() {
    ...
    try { localStorage.setItem("deepen_" + receiptId + "_" + step, updated); } catch(e) { /* ignore */ }
    setEditing(false);
  }
  ...
  async function loadStep(s: number) {
    ...
    try {
      ...
      setStep(s);
      try { localStorage.setItem(cacheKey, result); } catch(e) { /* ignore */ }
    } catch (e) {
      setResults(prev => ({ ...prev, [s]: "오류: " + (e instanceof Error ? e.message : String(e)) }));
      setStep(s);
    }
    setLoading(false);
  }
...
function VerdictCard({ verdict, receiptId, reason, pov, notionSaved }: { verdict: string; receiptId: string; reason: string; pov: string; notionSaved: NotionSaveResult | null | undefined }) {
...
function HistoryItem({ r, onClick }: { r: Receipt; onClick: () => void }) {
  ...
  return (
    <div onClick={onClick} style={{ padding: 8, borderRadius: 10, cursor: "pointer", border: "1px solid transparent", transition: "all 0.15s" }}
      onMouseEnter={e => { (e.currentTarget as HTMLDivElement).style.background = "#F8FAFC"; (e.currentTarget as HTMLDivElement).style.borderColor = "#F1F5F9"; }}
...
export default function AXAgentChat() {
  ...
  const [notionSaved, setNotionSaved]         = useState<NotionSaveResult | undefined>(undefined);
  const [showDeepen, setShowDeepen]           = useState(false);
  const [deepenData, setDeepenData]           = useState<{ answers: Answers; pov: string; receiptId: string; verdict: string } | null>(null);
  const [followupQueue, setFollowupQueue]     = useState<string[]>([]);
  const [followupIdx, setFollowupIdx]         = useState(0);
  const [povCandidates, setPovCandidates]     = useState<PovCandidate[]>([]);
  ...
  const [suggestAnswers, setSuggestAnswers]   = useState<Answers>({});
  ...
  const povRef             = useRef("");
  const pendingContextRef  = useRef<{ rId: string; modifiedAnswers: Answers; sessName: string; sessTeam: string } | null>(null);
  ...
  function addMsg(from: string, content: ReactNode, extra: Partial<ChatMessage> = {}) {
    setMessages(prev => [...prev, { from, content, id: Date.now() + Math.random(), ...extra }]);
    setIsTyping(false);
  }
  function agentSay(content: ReactNode, delay = 600) {
    setIsTyping(true);
  ...
  function proceedInterview(text: string, qId: string) {
    const newAnswers = { ...answers, [qId]: text };
    setAnswers(newAnswers); setInput("");
  ...
  function goToPovConfirm(ans: Answers) {
    setSuggestedQs([]);
    setSuggestAnswers({});
  ...
  function proceedWithVerdict(pov: string) {
    ...
    <div style={{ display: "flex", height: "100vh", background: "#F8FAFC", fontFamily: "'Pretendard','Apple SD Gothic Neo',-apple-system,sans-serif", overflow: "hidden" }}>
      ...
      <div style={{ width: 240, background: "white", borderRight: "1px solid #F1F5F9", display: "flex", flexDirection: "column", flexShrink: 0 }}>
        ...
        <div style={{ padding: "0 16px", display: "flex", gap: 4, borderBottom: "1px solid #F1F5F9" }}>
          {([["history","접수 히스토리"],["sprint","스프린트"]] as const).map(([tab, label]) => (
